...
// ============= NEW: ANTI-DELETE STYLED MESSAGE =============
async function sendStyledAntiDelete(sock, ownerJid, deletedMsg, sender) {
  try {
    ...
    if (deletedMsg.message.conversation) {
      ...
      msg += `🖼️ Image: ${deletedMsg.message.imageMessage.caption || 'No caption'}\n`;
    } else if (deletedMsg.message.videoMessage) {
      msg += `🎥 Video: ${deletedMsg.message.videoMessage.caption || 'No caption'}\n`;
    } else if (deletedMsg.message.stickerMessage) {
      msg += `🎨 Sticker was deleted\n`;
    } else if (deletedMsg.message.audioMessage) {
      msg += `🎵 Audio was deleted\n`;
    } else {
...
setTimeout(() => {
  async function _0x1b1480() {
    ...
    // ============= FIXED ANTI-DELETE WITH STYLED MESSAGE =============
    _0x243e88.ev.on("messages.upsert", async _0x43b2d7 => {
      if (conf.ANTIDELETE1 === "yes") {
        ...
        if (!_0x213692.endsWith('@g.us')) return;
        ...
        if (_0x20b50c.message.protocolMessage && _0x20b50c.message.protocolMessage.type === 0x0) {
          ...
          if (_0x475212) {
            try {
              ...
              // Send styled anti-delete message
              await sendStyledAntiDelete(_0x243e88, ownerJid, _0x475212, sender);
